...
export const videoSlice = createSlice(<VideoSlice>{
  ...
  reducers: {
    addVideos: (state: VideosBlockOption, action: VideoAction) => {
      ...
      state.videosBlock.videos = [
        ...state.videosBlock.videos,
        action.payload?.dataVideo?.videos || [],
      ];
    },
    addVideoSearch: (
      state: VideoSearchBlockOption,
      action: VideoSearchAction
    ) => {
      state.videoSearchBlock.loadingVideo = action.payload.loading;
      state.videoSearchBlock.totalResult =
        action.payload.dataVideo?.total_results;
...
export const selectVideoDetail = (state: StateInterface) =>
  state.videos.videosDetails;
export const selectVideoSearchBlock = (state: StateInterface) =>
  state.videos.videoSearchBlock;
